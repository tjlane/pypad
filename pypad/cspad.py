...
class CSPad(object):
    ...
    def do_asics_overlap(self):
        ...
        for i in range(64):
            ...
            path = Path(asic)
            in_area = path.contains_points(others)
            any_inside = np.sum(in_area).astype(np.bool)
            if any_inside:
                asics_overlap = True                
...
class XppCSPad(CSPad):
    
    def __init__(self, metrology, quad_offset=np.zeros((4,2)), 
                 quad_rotation=np.zeros(4), verbose=True):
        
        super(XppCSPad, self).__init__(metrology, quad_offset, 
                                       quad_rotation, verbose)
        
        # xpp metrology is way off from center -- get it close
    ...
    def _twobyone_to_bg(self, quad_metrology, quad_index, two_by_one_index):
        ...
        # the three-letter code xxY stands for the corner of the first pixel
        # (ie. xx = bl = bottom left) and the capital letter stands for the
        # orientation of the 2x1 (V = vertical, H = horizontal)
        
        if quad_index == 0:
            scan_sequence = np.array([3,2,5,4,7,6,0,1])
            corner = ['blV', 'blV', 'ulH', 'ulH', 'urV', 'urV', 'ulH', 'ulH']
        elif quad_index == 1:
            scan_sequence = np.array([1,0,3,2,4,5,7,6])
            corner = ['ulH', 'ulH', 'urV', 'urV', 'brH', 'brH', 'urV', 'urV']
        elif quad_index == 2:
            scan_sequence = np.array([6,7,1,0,2,3,4,5])
            corner = ['urV', 'urV', 'brH', 'brH', 'blV', 'blV', 'brH', 'brH']
        elif quad_index == 3:
            scan_sequence = np.array([5,4,6,7,0,1,2,3])
            corner = ['brH', 'brH', 'blV', 'blV', 'ulH', 'ulH', 'blV', 'blV']
        ...
        if corner[two_by_one_index] == 'ulH':
        ...
        elif corner[two_by_one_index] == 'urV':
        ...
        elif corner[two_by_one_index] == 'brH':
        ...
        elif corner[two_by_one_index] == 'blV':

            s = 0.10992 * self._unit( (xyz_2x1[3,:] - xyz_2x1[0,:]) +
                                      (xyz_2x1[1,:] - xyz_2x1[1,:]) )
            f = 0.10992 * self._unit( (xyz_2x1[2,:] - xyz_2x1[3,:]) +
                                      (xyz_2x1[1,:] - xyz_2x1[0,:]) )


            diagonal = (xyz_2x1[2,:] - xyz_2x1[0,:]) / 1000.
            center = np.sum(xyz_2x1, axis=0) / 1000.
            offset = (np.linalg.norm(diagonal) - h) / 2.

            p0 = xyz_2x1[0,:] / 1000. +  offset * self._unit(center)
            p1 = p0 + shape[1] * f + 0.27480 * self._unit(f) # for 3px gap
