...
parser.add_option("-t", "--threshold", action="store", type="float", dest="threshold", 
					help="threshold (ADU) below which automasking is performed (default = 0 ADUs)", metavar="THRESHOLD", default="0")
parser.add_option("-r", "--rowthirteen", action="store_true", dest="row13", 
					help="automatically mask out row13 of CSPAD (default: false)", default=False)
...
mask=Make_mask(d,options.maskName,options.threshold,options.row13)
