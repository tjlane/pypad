...
#    ./viewAvgIntensities.py -rxxxx -m 400 (optional) -x 800 (optional)
# For details, type 
#	 python viewAvgIntensities.py --help
...
parser.add_option("-m", "--min", action="store", type="float", dest="min_value", 
					help="ignore intensities below this event", metavar="min_value", default="-1")
parser.add_option("-x", "--max", action="store", type="float", dest="max_value", 
					help="ignore intensities above this event", default="0")
...
print source_dir+runtag+"/"+runtag+"-intensities.h5"
f = H.File(source_dir+runtag+"/"+runtag+"-intensities.h5","r")
...
if options.max_value == 0:
    options.max_value = len(intensities)
iav = N.average(intensities)
istd = N.std(intensities)

print "Average avg intensity = %s ADU, std = %s ADU"%(iav, istd)
